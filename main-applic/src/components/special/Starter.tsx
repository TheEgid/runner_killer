...
interface Props {
    ...
    autoDeleteRuns?: boolean
}

const PipelineStarter: React.FC<Props> = ({ deploymentName, autoDeleteRuns = false }) => {
    const { deploymentId, error: deploymentError } = useDeployment(deploymentName);
    const { runId, status, logs, runtime, loading, error, startFlow, stopFlow } = useFlowRun(autoDeleteRuns);

    const stopLoading = loading && status === "RUNNING";
    const anyError = deploymentError || error || (logs.length === 0 && status === "FAILED" ? "Ошибка выполнения" : undefined);
