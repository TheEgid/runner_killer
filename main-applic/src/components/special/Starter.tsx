...
import { ACTIVE_STATUSES, getBadgeColor, LogsCard, STOPPABLE_STATUSES } from "./Helpers";
...
const PipelineStarter: React.FC<Props> = ({ deploymentName }) => {
    ...
    const anyError = deploymentError || error;
        ...
        <>
            ...
            <Stack mt="md" gap="md">
                <Group justify="apart">
                    ...
                    {status && status !== "NOT_STARTED" && <Badge color={getBadgeColor(status)}>{status}</Badge>}
                    {status === "RUNNING" && (
